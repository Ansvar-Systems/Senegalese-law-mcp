#!/usr/bin/env tsx
/**
 * Database builder for Senegal Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Senegal Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Senegalese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'SN');
    insertMeta.run('source', 'primature.sn / africa-laws.org');
