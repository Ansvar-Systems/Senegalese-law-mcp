#!/usr/bin/env tsx
/**
 * Senegalese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://jo.gouv.sn';
const PORTAL_NAME = 'Senegal Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/senegalese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Senegalese Law MCP — Data Freshness Check');
