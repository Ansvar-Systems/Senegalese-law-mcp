#!/usr/bin/env tsx
/**
 * Drift detection for Senegal Law MCP.
 *
 * Checks if upstream Senegal Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Senegal Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'senegalese-law-mcp/1.0 drift-detect' },
      });
