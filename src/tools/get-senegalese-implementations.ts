/**
 * get_senegalese_implementations — Find Senegalese statutes implementing a specific EU directive/regulation
...
export interface GetSenegaleseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SenegaleseImplementationResult {
  document_id: string;
...
export async function getSenegaleseImplementations(
  db: InstanceType<typeof Database>,
  input: GetSenegaleseImplementationsInput,
): Promise<ToolResponse<SenegaleseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SenegaleseImplementationResult[];
