/**
 * search_eu_implementations — Search EU directives/regulations that have Senegalese implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_senegalese_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  senegalese_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as senegalese_statute_count
  ...
  if (input.has_senegalese_implementation) {
    sql += ' HAVING senegalese_statute_count > 0';
  }
